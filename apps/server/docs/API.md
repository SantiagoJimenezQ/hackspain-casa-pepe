...
`mode=llm` sends the deterministic ranking and the customer state to the fast model (`LLM_FAST_MODEL`, or `LLM_MODEL` when empty) and asks it to confirm or reorder the customers with one sentence of `justification` each. The answer is cached per run until a service status, recovery action or blocked dependency changes, so repeated reads are instant. `source` says whether the order came from the `llm` or is the `deterministic` fallback, `model` names the model used, and `fallbackReason` explains a fallback (timeout past `LLM_FAST_TIMEOUT_MILLISECONDS`, provider error or an invalid answer).
