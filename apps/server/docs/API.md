...
### Recorded-demo controls

- `POST /api/demo/capacity` (operator auth): `{ runIdentifier, resourceIdentifier, totalCapacity, reason }`. Explicit run/resource required. Capacity is total usable units, including allocations. Rejects inactive/replay runs, unknown resources, blank reasons, fractions, negatives and reductions below allocated units. Persists before/after evidence and triggers the normal agent reassessment.
- `POST /api/recovery/probe` (operator auth): `{ runIdentifier }`. Requires an active manual run and HTTP recovery mode. Returns `{ runIdentifier, checkedAt, mode: "http", verified, detail, deliveryIdentifier, routeIdentifier }`. A failed functional check returns HTTP 200 with `verified: false`; unsupported mode returns 409. This endpoint does not mutate service health or resolve the incident.
- `GET /api/overview` adds `planComparison` (nullable, current/previous persisted plan snapshots, trigger, capacity changes and superseded approvals) and `deliveryProbes` (last two functional checks in chronological order). See `packages/contracts/demo-controls.d.ts` for the shared payloads.
