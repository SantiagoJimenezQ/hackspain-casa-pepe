...
describe.each(["legacy", "combined", "compact"])(
	"%s: agent flow (integration with in-memory repositories)",
	(mode) => {
		...
		beforeAll(async () => {
			...
			Object.assign(application.get(ConfigurationService).agent, {
				combinedPlanActionEnabled: mode !== "legacy",
				compactPlanEnabled: mode === "compact",
			})
		...
		it("runs the whole demo: impact, call, twist, approval, recovery, verification and learning", async () => {
			...
			expect(
				planTwo.priorities.find(
					(priority) =>
						priority.serviceIdentifier === "package-tracking",
				)?.decision,
			).toBe("recover-now")
			expect(
				planTwo.priorities.find(
					(priority) =>
						priority.serviceIdentifier === "events-stream",
				)?.decision,
			).toBe("recover-now")
			expect(
			...
			expect(
				firstFlowCalls.map(({ call }) => call.function.name),
			).toEqual(
			...
			expect(
				new Set(firstFlowCalls.map(({ call }) => call.id)).size,
			).toBe(firstFlowCalls.length)
		...
		it("respects an operator rejection and keeps the service postponed in the revised plan", async () => {
			...
			expect(
				revised.priorities.find(
					(priority) =>
						priority.serviceIdentifier === "route-assignment",
				)?.decision,
			).toBe("postpone")
			expect(
