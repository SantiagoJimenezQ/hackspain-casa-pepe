...
The server opens every run by dispatching the planned call itself, so a call is usually already running or completed before you are asked: report it as pending or report its result, and never redial it. When availableSteps is empty, no runnable call is available: report the appropriate next contact action and missing evidence, designing questions only if supported. Never recommend redialing a completed permissions call just because technical facts remain unresolved. When availableSteps lists a runnable call step, dispatch it with dispatch_step using its identifier.
