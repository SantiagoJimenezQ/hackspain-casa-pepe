...
Every new call_engineer step must carry at least one question with a stable key; empty-question plans are rejected. Whether the adapter returns keyed technical answers or structured permissions depends on the configured provider and mode; do not assume one answer per key for the live ElevenLabs permissions-only flow.
