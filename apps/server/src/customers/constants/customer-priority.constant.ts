...
/**
 * The companies the operator always wants attended first, in this exact order, whatever their
 * scores or sectors say. Anything not listed here queues behind them.
 */
export const CUSTOMER_PRIORITY_ORDER: ReadonlyArray<string> = [
	"purehealth",
	"emirates-nbd",
	"happyrobot",
]
...
The head of the order is fixed and not yours to change: ${CUSTOMER_PRIORITY_ORDER.join(", then ")} come first, in that order, and after them every health customer, then every finance customer, then every logistics one, and only then the rest. Within a sector, decide the final recovery order: keep the baseline unless you see a concrete reason to move a customer, such as a dependency that unblocks several customers at once, a recovery already running, or a disproportionate user impact.
...
export const CUSTOMER_PRIORITY_CRITERIA = [
	{
		description: `Fixed company order, before anything else: ${CUSTOMER_PRIORITY_ORDER.join(", ")}`,
		key: "company-order",
	},
	{
		description:
