...
describe("meteorite gulf geography", () => {
	it("places the impact in Dubai and orders backups Oman then Riyadh then Frankfurt", () => {
		...
		).toEqual(["muscat-lz", "eu-central-1", "riyadh"])
	...
	it("selects Frankfurt when Oman no longer has enough remaining capacity", () => {
		const selected = selectBackupResource(createImpactedIncident(1))
		expect(selected.identifier).toBe("backup-frankfurt")
		expect(selected.region).toBe("eu-central-1")
	})
