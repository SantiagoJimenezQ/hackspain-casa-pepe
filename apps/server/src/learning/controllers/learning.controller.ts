...
export class LearningController {
	...
	@ApiOperation({ summary: "What the agent learned from previous runs and applies to new plans" })
	insights(@Query("scenarioIdentifier") scenarioIdentifier?: string): Promise<ReadonlyArray<LearningInsightRecord>> {
		return this.learningService.list(scenarioIdentifier)
	}

	@Delete("insights")
	@HttpCode(HttpStatus.OK)
	@ApiOperation({ summary: "Forget every insight so the next run starts without prior knowledge" })
	...
	async currentReport(): Promise<RunReport> {
		return this.runReportService.build(await this.runsService.resolveRunIdentifier())
	}

	@Get("reports/:runIdentifier")
	@ApiOperation({ summary: "Post-incident report of a run: durations, plan versions, approvals, outcomes and lessons" })
