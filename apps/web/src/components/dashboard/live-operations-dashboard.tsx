...
function Companies() {
  const { overview } = useDashboard();
  if (!overview) return null;
  const customers = customerView(overview.incident);
  return (
      ...
      <div className="scroll-fade-x flex gap-2 overflow-x-auto pb-0.5">
        {(overview.incident.services ?? []).map((service) => (
          <motion.div
            ...
            layout
            className="min-w-[150px] rounded-lg border border-border/80 px-3 py-2"
            animate={{ borderColor: `color-mix(in srgb, ${COLORS[statusOf(service.status)]} 35%, var(--border))` }}
          >
            <div className="flex items-center gap-1.5 text-[11px] font-medium">
              <Status value={service.status} />
              {service.name}
            </div>
            <p className="mt-1 truncate text-[10px] text-muted-foreground">{service.statusReason}</p>
          </motion.div>
        ))}
      </div>
    </Panel>
...
export function LiveOperationsDashboard() {
  const { status, overview, error, retry, startDemo, busyAction } = useDashboard();
    ...
    return (
      <div className="flex flex-1 items-center justify-center">
        <Panel className="max-w-xl p-8">
          ...
          <h1 className="mt-3 text-3xl font-semibold">{status === "error" ? "No se puede alcanzar al coordinador" : "La simulación está preparada"}</h1>
          <p className="mt-3 text-sm text-muted-foreground">{error ?? "Inicia la crisis de los EAU para observar y coordinar la recuperación."}</p>
          <Button className="mt-6" onClick={() => void (status === "error" ? retry() : startDemo())} disabled={busyAction !== null}>
            {status === "error" ? "Reintentar" : "Iniciar demo"}
          </Button>
        </Panel>
      </div>
    );
